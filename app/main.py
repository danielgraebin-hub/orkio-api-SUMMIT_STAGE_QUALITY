...
async def stt_endpoint(
    ...
    try:
        ...
        try:
            ...
            return {"text": text, "language": (requested_language or "auto"), "trace_id": trace_id}
        finally:
            try:
                import os as _os
                _os.unlink(tmp_path)
...
def realtime_get_session_ata(
    ...
    """Export a plain-text transcript for a realtime session."""
    org = _resolve_org(user, x_org_slug)
    uid = user.get("sub")
    ...
    evs = db.execute(
        ...
        .where(
            RealtimeEvent.org_slug == org,
            RealtimeEvent.session_id == session_id,
            RealtimeEvent.event_type.in_(["transcript.final", "response.final"]),
        )
        .order_by(RealtimeEvent.created_at.asc(), RealtimeEvent.id.asc())
    ).scalars().all()
    ...
    def _event_text(ev: RealtimeEvent) -> str:
        if (ev.event_type or "") == "transcript.final":
            return _safe_text(getattr(ev, "transcript_punct", None) or ev.content)
        return _safe_text(ev.content)
    ...
    for ev in evs:
        ...
        if not body:
            continue
        role_label = "User" if (ev.role or "").lower() == "user" else (ev.agent_name or rs.agent_name or "Orkio")
        dedupe_key = ((ev.event_type or "").strip(), role_label.strip().lower(), body.strip().lower())
        if dedupe_key == last_key:
            continue
        last_key = dedupe_key
        ts = ev.created_at or rs.started_at or now_ts()
        lines.append(f"[{ts}] {role_label}: {body}")

    if len(lines) == 7:
        lines.append("[info] No final transcript/response events were found for this session yet.")
